...
v1 :: AuthHeaders -> NW.Options
v1 = buildHeaders "1"


v2 :: AuthHeaders -> NW.Options
v2 = buildHeaders "2"


v3 :: AuthHeaders -> NW.Options
v3 = buildHeaders "3"


buildHeaders :: Text -> AuthHeaders -> NW.Options
buildHeaders version (AuthHeaders c x k _) = 
  baseHeaders version k & header "CST" .~ [cs c]
                        & header "X-SECURITY-TOKEN" .~ [cs x]


baseHeaders :: Text -> Text -> NW.Options
...
instance ToJSON InstrumentExpiry where
  ...
  toJSON None = String "-"
  toJSON x = String . cs . Text.Show.show $ x
