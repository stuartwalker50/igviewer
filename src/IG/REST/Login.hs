...
--    (apiKey, loginDetails) <- getCredentials isDemo
    let (apiKey, loginDetails) = ("959fe13b9a3a1967f438c3affcf618893966497c", LoginBody False "snwdemo1" "Target01")
...
--  response <- postWith (loginOptions key) loginUrl (toJSON logBody)
  let a = (loginOptions key)
  let b = loginUrl
  let c = (toJSON logBody)
--  print a
--  print b
--  print c
  response <- postWith a b c
